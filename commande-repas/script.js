/* SAJ ANAGALLIS — WIDGET GRIST COMMANDES REPAS
   Version V7 : semaines indépendantes, contrôles, historique des modifications,
...
const DIETS=['Normal','Hypocalorique','Hypolipidique','Sans porc','Sans viande'];
...
async function seedConfigFromSources(){
  ...
  sourceUsers.forEach(r=>{
    ...
    actions.push(['AddRecord',TABLES.config,null,{PersonKey:key,SourceType:'Usager',SourceId:r.id,Nom:r.Nom||'',Prenom:r.Prenom||'',Groupe:inferFloor(r.id),Regime:r.Regime||'Normal',Texture:'Normale',Lu:!!r.Lu,Ma:!!r.Ma,Me:!!r.Me,Je:!!r.Je,Ve:!!r.Ve,DateDebut:'',DateFin:'',Actif:true}]);
  });
...
async function archivePastWeeks(){const today=mondayOf(new Date());const a=[];weeks.forEach(w=>{if(parseKey(w.SemaineKey)<today&&w.Statut!=='Archivée')a.push(['UpdateRecord',TABLES.weeks,w.id,{Statut:'Archivée'}])});if(a.length)await grist.docApi.applyUserActions(a)}
...
function renderWeekNavigation(){
  ...
  $('weekList').innerHTML=sorted.slice(0,24).map(w=>`<button class="week-link ${w.SemaineKey===weekKey(weekStart)?'active':''} ${weekAlertClass(w)} ${w.Rectificative?'rectificative':''}" data-open-week="${w.SemaineKey}"><span>${weekShort(parseKey(w.SemaineKey))}</span><span class="status ${w.Rectificative?'rectificative':statusClass(w.Statut)}">${esc(w.Rectificative?'Rectificative':(w.Statut||'À préparer'))}</span></button>`).join('')||'<p class="hint">Aucune semaine enregistrée pour cette année.</p>';
  document.querySelectorAll('[data-open-week]').forEach(b=>b.onclick=()=>openWeek(b.dataset.openWeek));
}
function renderCommande(){
  const wk=currentWeek();const label=weekLabel(weekStart);$('weekTitle').textContent=label;$('weekStatus').value=wk?.Statut||'À préparer';$('weekComment').value=wk?.Commentaire||'';
  ...
  const archived=wk?.Statut==='Archivée';
...
function summaryGroup(group,band,totalClass,c){
  ...
  const defs=[['Normal','diet'],['Hypocalorique','diet'],['Hypolipidique','diet'],['Sans porc','diet'],['Sans viande','diet'],['Purée lisse','texture'],['Haché lubrifié','texture']];
...
function renderTemplateEditor(){
  ...
  root.innerHTML=`<table class="settings-table template-table"><thead><tr><th>Nom – Prénom</th><th>Groupe</th><th>Régime</th><th>Texture</th>${DAYS.map(d=>`<th>${d.short}</th>`).join('')}<th>Note cuisine habituelle</th></tr></thead><tbody>${people.map(p=>{
...
async function saveWeekStatus(){const w=currentWeek();if(!w)return;const old=w.Statut||'À préparer',status=$('weekStatus').value;const now=new Date(),iso=now.toISOString(),fields={Statut:status,ModifieLe:iso,ModifieLeDT:gristDateTime(now)};if(status==='Commandée'){fields.CommandeeLe=iso;fields.CommandeeLeDT=gristDateTime(now)}await grist.docApi.applyUserActions([['UpdateRecord',TABLES.weeks,w.id,fields]]);await logAudit({action:'Statut',oldValue:old,newValue:status,detail:`Statut : ${old} → ${status}`});await loadAll();renderAll();saved()}
async function saveCurrentWeekExplicitly(){
  ...
  try{
    ...
    if(status!==(w.Statut||'À préparer')){fields.Statut=status;if(status==='Commandée'){fields.CommandeeLe=now.toISOString();fields.CommandeeLeDT=gristDateTime(now)}}
...
function renderDashboard(){
  ...
  const state=w?.Rectificative?'Rectificative':w?.Statut||'À préparer';
...
function weekAlertClass(w){
  if(w.Statut==='Archivée'||w.Statut==='Commandée')return'';
